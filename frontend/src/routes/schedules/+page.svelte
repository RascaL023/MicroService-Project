<script lang="ts">
	...
	import type { Group, GroupSchedule, LoginData, PageData, PaginationMeta, ScheduleTemplate } from '$lib/types';
	...
	const canManage = $derived(hasAnyAuthority(session, [
	...
	async function loadAll() {
		loading = true;
		try {
			const [groupPayload, templatePayload] = await Promise.all([
				api<PageData<Group> | Group[]>('/api/groups?status=ON_GOING&sort=name,asc'),
				api<PageData<ScheduleTemplate> | ScheduleTemplate[]>('/api/schedule-templates?sort=startTime,asc')
			]);
	...
	async function loadSchedules() {
		...
		const payload = await api<PageData<GroupSchedule> | GroupSchedule[]>(`/api/group-schedules?${query}`);
		...
		<section class="action-grid">
			<div class="panel">
				<div class="panel-head">
					...
					<Icons name="calendar" size={22} />
				</div>
				<form class="stack-form" onsubmit={(event) => { event.preventDefault(); void addSchedule(); }}>
					<label>
						<span>Group</span>
					...
					{#if templates.length === 0}
						<div class="soft-note">Belum ada template waktu. Buat template dulu di panel sebelah.</div>
					{/if}
			...
			<div class="panel">
				<div class="panel-head">
					...
					<Icons name="clock" size={22} />
				</div>
				<form class="stack-form" onsubmit={(event) => { event.preventDefault(); void createTemplate(); }}>
					<label>
					...
					<button class="btn btn-secondary" type="submit">Buat Template</button>
				</form>
			</div>
		</section>
	{/if}
