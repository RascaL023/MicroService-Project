...
func (r *UserRepository) Create(ctx context.Context, username, hashPassword string, roleIDs []int64) (entity.User, error) {
	...
	err = tx.QueryRow(ctx, `
		...
		VALUES ($1, $2, now())
		RETURNING id, username, hash_password, is_banned, created_at
	`, username, hashPassword).Scan(&user.ID, &user.Username, &user.HashPassword, &user.IsBanned, &user.CreatedAt)
	if err != nil {
...
func (r *UserRepository) List(ctx context.Context, limit, offset int) ([]entity.User, int, error) {
	...
	rows, err := r.pool.Query(ctx, `
		SELECT id, username, hash_password, is_banned, created_at
		FROM users
	...
	for rows.Next() {
		var user entity.User
		if err := rows.Scan(&user.ID, &user.Username, &user.HashPassword, &user.IsBanned, &user.CreatedAt); err != nil {
			return nil, 0, err
		}
		user.Roles, err = r.rolesForUser(ctx, user.ID)
		if err != nil {
...
func (r *UserRepository) findOne(ctx context.Context, where string, arg any) (entity.User, error) {
	...
	err := r.pool.QueryRow(ctx, `
		SELECT u.id, u.username, u.hash_password, u.is_banned, u.created_at
		FROM users u
		`+where, arg).Scan(&user.ID, &user.Username, &user.HashPassword, &user.IsBanned, &user.CreatedAt)
...
func (r *UserRepository) rolesForUser(ctx context.Context, userID int64) ([]entity.Role, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT r.id, r.role, r.created_at
		FROM roles r
...
func (r *UserRepository) authoritiesForRole(ctx context.Context, roleID int64) ([]string, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT a.name
		FROM authorities a
...
func (r *UserRepository) CreateSession(ctx context.Context, token string, userID int64, expiresAt time.Time) error {
	_, err := r.pool.Exec(ctx, `INSERT INTO sessions (token, user_id, expires_at) VALUES ($1, $2, $3)`, token, userID, expiresAt)
	return err
}
