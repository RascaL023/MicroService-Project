...
func SeedAdmin(ctx context.Context, cfg config.Config, users *repository.UserRepository, roles *repository.RoleRepository) error {
	...
	if _, err := ensureRole(ctx, roles, "USER"); err != nil { return err }

	exists, err := users.ExistsByUsername(ctx, cfg.AdminUsername)
	if err != nil { return err }
	...
	_, err = users.Create(ctx, cfg.AdminUsername, string(hash), []int64{adminRole.ID})
	if errors.Is(err, repository.ErrDuplicate) { return nil }
...
func ensureRole(ctx context.Context, roles *repository.RoleRepository, name string) (entity.Role, error) {
	...
	if errors.Is(err, repository.ErrNotFound) { role, err = roles.Create(ctx, name, nil) }
